...
import { CalendarDays, Tag, ChevronRight, Plus, Pencil, Trash2 } from 'lucide-react';
import { useData } from '../context/DataContext';
import { useAdmin } from '../context/AdminContext';
import EditNoticiaModal from './admin/EditNoticiaModal';
...
function FeaturedCard({ noticia, visible, isAdmin, onEdit, onDelete }) {
  return (
    <div className={`fade-up ${visible ? 'visible' : ''} news-card group relative rounded-2xl overflow-hidden cursor-pointer`}>
      ...
      <div className="news-img-wrap aspect-[16/9] sm:aspect-[16/8]">
        <img src={noticia.imagen} alt={noticia.titulo} className="news-img w-full h-full object-cover" loading="lazy" />
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
        ...
        <h3 className="news-card-title text-xl sm:text-2xl font-bold text-white leading-snug mb-3 line-clamp-3">{noticia.titulo}</h3>
        <p className="text-gray-300 text-sm leading-relaxed line-clamp-2 mb-4">{noticia.resumen}</p>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-xs text-gray-400">
            <CalendarDays size={12} /><span>{noticia.fecha}</span>
          </div>
...
function SmallCard({ noticia, delay, visible, isAdmin, onEdit, onDelete }) {
  return (
    <div className={`fade-up delay-${delay} ${visible ? 'visible' : ''} news-card group glass rounded-2xl overflow-hidden border border-white/5 hover:border-blue-500/20 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-900/20 cursor-pointer flex flex-col relative`}>
      {isAdmin && <AdminControls onEdit={onEdit} onDelete={onDelete} />}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <h3 className="news-card-title text-sm font-bold text-white leading-snug flex-1 line-clamp-3 mb-3">{noticia.titulo}</h3>
...
export default function Noticias() {
  ...
  const [editTarget, setEditTarget] = useState(null); // noticia | 'new' | null
  const { noticias, setNoticias } = useData();
  ...
  const handleSave = (noticia) => {
    setNoticias(
      noticias.some((n) => n.id === noticia.id)
        ? noticias.map((n) => n.id === noticia.id ? noticia : n)
        : [noticia, ...noticias]
    );
  };

  const handleDelete = (id) => {
    if (confirm('¿Eliminar esta noticia?')) setNoticias(noticias.filter((n) => n.id !== id));
  };
    ...
    <section id="noticias" ref={sectionRef} className="py-24 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`fade-up ${visible ? 'visible' : ''} flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10`}>
          <div>
            <span className="text-xs font-bold tracking-widest uppercase text-blue-400 mb-3 block">Actualidad Sansera</span>
            <h2 className="font-display text-5xl sm:text-6xl text-white tracking-wider">NOTICIAS</h2>
          </div>
          {isAdmin && (
            <button
              onClick={() => setEditTarget('new')}
              className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-full transition-all duration-200 hover:scale-105"
            >
              <Plus size={15} /> Añadir noticia
            </button>
          )}
        </div>
          ...
          {categorias.map((cat) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 ${
                filter === cat ? 'bg-blue-600 text-white shadow-md shadow-blue-600/25' : 'glass text-gray-400 hover:text-white border border-white/5 hover:border-blue-500/20'
              }`}
            >
              {cat}
            </button>
        ...
        {filtradas.length > 0 ? (
          <>
            {destacada && (
              <div className="mb-8">
                <FeaturedCard
                  noticia={destacada} visible={visible} isAdmin={isAdmin}
                  onEdit={() => setEditTarget(destacada)}
                  onDelete={() => handleDelete(destacada.id)}
                />
              </div>
            )}
            {resto.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {resto.map((n, i) => (
                  <SmallCard
                    key={n.id} noticia={n} delay={Math.min(i + 1, 5)} visible={visible} isAdmin={isAdmin}
      ...
      {editTarget && (
        <EditNoticiaModal
          noticia={editTarget === 'new' ? null : editTarget}
          onSave={handleSave}
